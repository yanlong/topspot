...
FortuneHistory = new Mongo.Collection('fortuneHistory');
...
if (Meteor.isServer) {
    ...
    Favors._ensureIndex({comment:1, topic: 1, type:1, user:1}, {unique: true});
    Follows._ensureIndex({user:1, mtime: 1});
    Follows._ensureIndex({target:1, mtime: 1});
    Follows._ensureIndex({user:1, target: 1}, {unique: true});
    Rankings._ensureIndex({type:1, isFinal:1, catalog: 1, settled: 1});
    Rankings._ensureIndex({type:1, topic:1, mtime:-1});
